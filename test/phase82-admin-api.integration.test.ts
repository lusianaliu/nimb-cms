...
const mkdtemp = () => fs.mkdtempSync(path.join(os.tmpdir(), 'nimb-phase82-'));
...
const writeInstallState = (cwd: string) => {
  ...
  fs.writeFileSync(path.join(nimbDir, 'install.json'), `${JSON.stringify({ installed: true, version: '82.0.0', installedAt: '2026-01-01T00:00:00.000Z' }, null, 2)}\n`);
};
...
test('phase 82: admin runtime bridge provides system info API', async () => {
  await withInstallState(async () => {
    markInstalled({ version: '82.0.0' });
    ...
    try {
      assert.deepEqual(bootstrap.runtime.adminApi, { basePath: '/admin-api' });
      ...
      assert.equal(systemPayload.name, 'Nimb');
